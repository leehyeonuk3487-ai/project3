...
def build_report(schedule: str = "경기도", collected: dict | None = None) -> str:
    ...
    w("## 0. 미션·윤리 (먼저 명시)")
    w(
        "- **미션**: 같은 예산으로 **더 많은 장병이 더 많은 혜택**을 받게 하는 것. "
        "우리가 추구하는 목표는 비용을 줄이는 게 아니라, 정해진 예산 안에서 **혜택을 키우는 것**이다.\n"
        "- **집단 단위 설계용**: 지자체·보험사의 보장 설계·예산 책정을 돕는 도구다. "
        "**개인의 사고를 예측하거나 개인별로 보험료를 차등하지 않는다.**\n"
        "- **고위험 배제 금지**: 예산이 빠듯해도 생명·중대장해 보장은 반드시 지키도록 "
        "**완전보장 하한선**을 두어, 비용 효율을 이유로 고위험군이 밀려나지 않게 했다.\n"
    )
    ...
    w(f"| **M2** | 개인 손상위험 학습(집단 위험요인 파악용) | 정확도(AUC) 최고 **{m2['kfold_best_auc']}** — 나이·성별만 쓴 기준({m2['holdout_baseline_auc']})을 못 넘어 **단순식 채택**(개인 예측 약함을 정직 보고) |")
    ...
    w(f"| **M6** | 시나리오·민감도 + 예산 최적화(혜택 최대화) | 정해진 예산으로 혜택 총량을 최대화. 지금 급부표에선 단순 배분과 결과가 같지만, 가정을 흔들면 최적화가 우위임을 입증 |")
    w("")
    ...
    eff = ai["efficiency"]
    w("### 2-2. AI 성능 검증 (정직한 2단 구조)")
    w(
        f"- **★(1단계) 지역 발생률 학습 모델 — 보험의 핵심인 상해(외인)사망에서 단순 방식을 실제로 이김**:\n"
        f"  · *예측 오차 감소*: 처음 보는 지역 시험에서 **{sp_red}%**, 미래 연도 시험에서 **{tp_red}%** "
        f"줄임(단순 비례·단순 평균 두 기준 모두보다 정확) → 채택.\n"
        f"  · *보정*: 예측값을 5구간으로 나눠 보면 예측률과 실제 관측률이 거의 일치(눈금 기울기 "
        f"**{ai['calib_slope_oos']}**, 1에 가까울수록 완벽).\n"
        f"  · *근거의 타당성*: 지역 건강습관(흡연·음주·운동부족·비만) 정보를 넣으면 예측 오차가 "
        f"**{ai['chs_deviance_reduction_pct']}% 더 감소** → 지역 위험요인이 예측에 실제로 기여함.\n"
        f"  · *불확실성*: 예측 범위 안에 실제가 든 비율 **{_fmt(ai['pi_coverage']*100,0)}%**(목표 95%) "
        f"— 지나치게 자신하지 않고 정직하게 표시.\n"
        f"- **지역 위험 회귀(LORO)** — 한 지역씩 빼고 맞힌 평균 오차 **{_fmt(loro['model_mae'],2)}** vs "
        f"전국평균 기준 **{_fmt(loro['baseline_mae'],2)}** (**{_fmt(loro['mae_improvement_pct'],1)}% 더 정확**), "
        f"예측 범위 적중 {_fmt(loro['pi_coverage']*100,0)}%.\n"
        f"- **(2단계) 개인 손상위험 모델** — 정확도(AUC) 최고 **{m2['kfold_best_auc']}**(≈0.56로 약함). "
        f"미래 시험에선 나이·성별만 쓴 기준({m2['holdout_baseline_auc']})도 넘지 못해 **단순식을 유지**했다. "
        f"**개인 예측이 약하다는 사실을 그대로 공개** — 개인을 점수 매기지 않고 집단 위험요인 파악에만 쓴다.\n"
        f"- **속도·재현성** — 모델 학습 {_fmt(eff['gbm_fit_ms'],0)}ms, 예산 최적화 계산 "
        f"**{eff['lp_solver_ms']}ms**로 매우 빠르고, 같은 입력이면 항상 같은 결과가 나온다(시드 고정)"
        f"{'' if eff['deterministic'] else '(주의: 비결정)'}.\n"
        f"- **차별점 = 검증의 깊이** — 지역 시험·미래 시험·보정·요인 기여도·예측 범위에 더해 "
        f"**국방부 실데이터 교차검증(5절)**까지. 대부분의 시제품이 생략하는 수준의 검증을 갖췄다.\n"
        f"- **정직 비교 원칙** — 모든 모델을 단순 기준과 나란히 보고하고, '단순식으로 충분하다'는 "
        f"결과도 그대로 인정한다(억지로 AI가 이기게 만들지 않음).\n"
    )
    w("**상해사망 예측 보정 점검(예측률 ≈ 실제 관측률, 기울기 0.99):**")
    ...
    w("### 2-3. 독창성")
    w(
        "- 실제 보험금 청구 자료가 없는 군 보험 위험을 **공개 통계만으로 수치화**했다"
        "(발생률로 청구액을 대신 추정) + 보장 항목별 보험료까지 산출.\n"
        "- **예산 최적화 — 미션에 맞춘 목표**: 비용을 줄이는 게 아니라 **정해진 예산 안에서 장병 "
        "혜택을 최대로** 키우도록 배분하고(선형계획법), 생명·중대장해는 반드시 지키는 하한선을 둔다.\n"
        "  · *왜 '머릿수'가 아니라 '가중 혜택'인가*: 단순히 혜택 인원만 최대화하면 값싼 보장을 잔뜩 "
        "끼워 넣는 쪽으로 치우친다. 우리 목표는 '더 많은 사람에게 값싼 혜택'이 아니라 '더 많은 장병이 "
        "**더 중대한 혜택**'을 받는 것이라, 사건의 중대도(사망>후유장해>경상)로 가중치를 준다. "
        "**이 가중치는 결과에서 거꾸로 맞춘 게 아니라, 국제 질병부담(GBD) 장애가중치 같은 독립 "
        "근거로 미리 정해 고정**했다.\n"
    )
    ...
    cliff = c["cliff"]
    w(
        f"- **고객**: 지자체(보험 가입 주체)와 보험사(보험료 산정) 양쪽에 쓸 수 있다.\n"
        f"- **인구 감소**: 전국 20대 남성 **{_fmt(cliff['y2024'])} → {_fmt(cliff['y2030'])}명 "
        f"({cliff['pct']}%, 2024→2030)**. 복무 인원이 줄면 미래 총예산도 달라진다(3-2의 다년 범위).\n"
        f"- **확장 계획**: 병무청 공개 API 실시간 연동, 군 훈련 손상의 실제 청구 자료를 확보하면 "
        f"추정값을 실측으로 전환, 통계청 안심구역 미시데이터로 개인 정보 보강.\n"
    )
    ...
    w(
        f"\n→ 미래 상해사망 추세는 **하나의 값이 아니라 범위(밴드)**로 제시한다: 낙관(과거 감소세 재개 "
        f"{tr['optimistic']}%/년) · 기준(최근 둔화 {tr['base']}%/년) · 보수(감소 멈춤 "
        f"{tr['conservative']}%/년). 인구는 통계청 중위 추계 **한 경로**를 쓰고, 범위의 폭은 "
        f"**상해사망 추세의 불확실성**에서 나온다. **과거 추세를 연장한 것이라 정책·인구·사회 변화는 "
        f"반영되지 않는다.**\n"
    )

    w("### 3-3. 예산 최적화 — 정직한 비교 (조작 방지 장치 포함)")
    lp = c["lp"]
    w(
        f"최적화의 목표를 **'혜택 인원' → '중대도로 가중한 혜택'**으로 바꿨다(미션에 맞춤). 예산 80%에서 "
        f"단순 배분(그리디)과 최적화(LP)의 가중 혜택은 "
        f"**{_fmt(lp['greedy_welfare'],1)} = {_fmt(lp['lp_welfare'],1)} (같음)**.\n\n"
    ...
    w("**① 가중치를 바꿔가며 시험 — 최적화가 특정 가중치에서만 이기는지 전부 공개:**")
    ws = c["weight_sens"]
    ...
    w("\n→ 원칙에 따라 정한 가중치(머릿수·GBD·중대도, 모두 골절≥입원)에선 **결과가 같고**, "
      "입원을 골절보다 높게 둔 경우에만 최적화가 앞선다 → '가중치를 주면 무조건 최적화가 이긴다'고 "
      "주장하지 않는다.\n")
    w("**② 가정을 흔드는 시험 — 단순 배분의 우선순위 가정만 바꾸면 최적화(LP)가 확실히 우위:**")
    pt = c["perturbation"]
    ...
    w("## 4. 정직성·한계 종합")
    w(
        "1. **개인 예측은 약하다** — 쓰인 건강조사에는 개인별 부상 기록이 없어 개인을 예측할 수 없다"
        "(정확도 AUC≈0.56). 집단 위험요인 파악에만 쓴다.\n"
        "2. **실제 청구 자료가 없다** — 보험금 청구 자료가 공개되지 않아 공개 **발생률로 대신 추정**한다. "
        "발생률을 청구액으로 바꾸는 과정에 오차가 있다.\n"
        "3. **미래 예측의 한계** — 미래 상해사망 추세는 과거 추세를 연장한 것이다. 학습 모델은 미래 "
        "연장이 어려워 이 부분만 별도로 계산했고, 구조 변화는 반영 못 하므로 **범위로만** 해석한다.\n"
        "4. **지역 단위 분석의 한계** — 지역 회귀는 17개 시도 묶음이라 개인에 적용할 수 없고, 표본이 "
        "작아 참고 수준이다(성능이 낮게 나온 경우도 그대로 공개).\n"
        "5. **미시데이터 미사용** — 통계청 안심구역 승인 자료는 아직 연동하지 않았다(향후 보강 여지).\n"
        "6. **인원 추정 오차** — 현역 인원은 인구추계×현역율×복무연수로 근사한다"
        "(경기 9.8만, 보고 6.8만~10.5만과 부합).\n"
        "7. **보험료 산정 원칙 고정** — 현재 보험료·예산은 **실측 데이터**로 고정한다. 학습 모델의 "
        "미래 연장은 시나리오 투영에만 쓰고 현재 가격에는 넣지 않는다.\n"
        "8. **최적화 동률을 정직하게 공개** — 지금 급부표는 하한선 위 선택 항목이 골절·입원 2개뿐이고 "
        "지급액도 거의 같아, 최적화와 단순 배분의 결과가 **같게** 나온다. 이를 숨기거나 가중치를 거꾸로 "
        "맞춰 이기게 만들지 않았다. 최적화의 가치는 (a) 미션에 맞는 목표, (b) 최적 보장, "
        "(c) 가정을 흔들어도 견고함으로 제시한다(3-3).\n"
        "9. **군 대리지표의 한계 — 검증·보정 방향까지 제시(5절)** — 일반인구 데이터를 군 집단의 "
        "대리지표로 썼다. 국방부 실데이터로 대조하니 자살은 군이 민간의 ~44%(과대추정), 상해는 추세는 "
        "함께 움직이나(상관 r≈0.88) 수준은 ~3배 과대였다. 보정계수는 **참고용으로만 보이고 보험료에는 "
        "넣지 않는다**.\n"
    )
    ...
    w(
        "지금까지 군 내부 자료가 없어 **일반인구 통계를 군 집단의 대리지표**로 썼다. 이 가정이 "
        "타당한지, 대회 개최기관인 국방부의 실데이터로 직접 **검증하고 보정 방향까지** 제시한다. "
        "**★현재 보험료(실측 기반)는 그대로 두고, 정직성·참고용으로만 병기한다.**\n"
    )
    sui, ext = mil["suicide"], mil["external"]
    w(f"- **자살(보험 면책)**: 군인 자살률은 민간의 **{sui['adopted_ratio']}배**(최근 5년; 전 기간 "
      f"{sui['full']}배) 수준 → 일반인구로 대신 쓰면 군 자살을 과대추정한다. {sui['direction']} "
      f"(2025년 민간 자살률은 미공개라 비율에서 제외{sui['excluded_years']}.)\n")
    w(f"- **상해(외인)**: 군 안전사고율과 일반인구 20대 남성 상해사망을 대조 — 추세는 강하게 함께 "
      f"움직이고(상관 r={ext['trend_r']}), 수준은 군/일반 **{ext['level_ratio']}배**(약 "
      f"{ext['overestimate_factor']}배 과대). → 대리지표는 **추세는 타당하나 수준은 과대추정**"
      f"(통제된 복무환경·집계 범위 차이).\n")
    w("- **군기사고(총기·폭행 등)**: 작전·징계 성격이라 일반 상해보험 보장과 성격이 달라, 상해에 "
      "합치지 않고 **검증에서 제외**했다(별도 범주).\n")
    w("- **한계**: 연 단위·전국 단위 자료라 지역 분해가 없고, 병력 수는 자살 건수÷자살률로 **역산**한 "
      "값이며, 민간 자살률은 전 연령 기준이다 → 전국 단위 참고 보정치로만 쓴다.\n")
    w(f"- **정리**: 일반인구 데이터를 군 대리지표로 쓰되, 그 한계를 **개최기관 실데이터로 직접 검증**해 "
      f"정직성과 공공데이터 활용을 함께 강화했다. (보험료 영향: {mil['pricing_impact']})\n")
